...
export class SubmissionForm extends Component {
    ...
    formType = () => {
        if (this.props.method === "POST") {
            return <h3>Choose "Add" in the Dropdown Menu to add a new Recipe</h3>
        }
        if (this.props.method === "PUT") {
            return <h3>Click "Update" in the Dropdown Menu to update a Recipe</h3>
        }
        if (this.props.method === "DELETE") {
            return <h3>Select "Delete" in the Dropdown Menu to delete a Recipe</h3>
        }
            ...
            return (<div style={{ width: "60%", marginLeft: 20 }}>
                ...
                <Form style={{ marginBottom: 10 }}>
                    <Form.Control size="sm" as="select" id="basic-nav-dropdown" name="foodInfo" onChange={this.setFormType}>
                        ...
                        <option value="foodInfo">Add</option>
                        <option value="foodInfo">Update</option>
                        <option value="foodInfo">Delete</option>
                    </Form.Control>
                </Form>
                {this.displayForm()}
            </div>
